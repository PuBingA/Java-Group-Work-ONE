...
public class ProjectVersionManagement implements Serializable {
    ...
    public ProjectVersionManagement(ProjectVersionManagement other) {
        this.versions = new HashMap<>();
        for (Map.Entry<String, ProjectVersion> entry : other.versions.entrySet()) {
            this.versions.put(entry.getKey(), new ProjectVersion(entry.getValue())); // 深拷贝 ProjectVersion
        }
        this.versionNames = new ArrayList<>(other.versionNames);
        this.deletedVersions = new HashSet<>(other.deletedVersions);
    }
    ...
    public void mergeBranches(String mergeName,ProjectVersionManagement pv,Map<String,String> saveConfiguration) throws Exception{
        if(pv==null){
            ...
            if(v_1==null){
                ...
                finalversion=new ProjectVersion(mergeName,"merge",timestampString,v_1.getFileNames());
            }
            else{
                // 合并文件名和哈希值，处理合并冲突
